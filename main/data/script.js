var gW=`ws:var ws;window.addEventListener("beforeunload",(event)=>{ws.close();event.preventDefault();event.returnValue="";});window.onload=function(){profileOptions([132,54,255,0,142,57,26,27,222,195]);fillProfiles();};function initWS(){ws=new WebSocket(gW);ws.onopen=onOpen;ws.onclose=onClose;ws.onmessage=onMessage;}
function onOpen(event){}
function onClose(event){setTimeout(initWS,2000);}
buttonStates=[0,0,0,0];sliderStates=[0,0,0];function processButton(elem){num=parseInt(elem.id.split("")[1]);if(num<(buttonStates.length+1))
{if(buttonStates[num-1]==1){document.getElementById(elem.id+"_c").style.color="red";buttonStates[num-1]=0;}else{document.getElementById(elem.id+"_c").style.color="green";buttonStates[num-1]=1;}}else if(num==(buttonStates.length+1)){document.getElementById(elem.id+"_c").style.color="green";setTimeout(function(){document.getElementById(elem.id+"_c").style.color="black"},250);}}
function fillPage(){return True;}

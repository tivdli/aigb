var ws;var gW="ws://"+window.location.hostname+"ws";profile_name_climate=["Tropical","Dry","Temperate","Continental"];profile_name_stage=["Seedling","Vegetative","Budding","Ripening"];profile_name_type=["Common","Leaf","Fruit","Flower"];profile_name_mode=["Phase","Stable","Custom","Stress"];window.addEventListener("beforeunload",(event)=>{ws.close();event.preventDefault();event.returnValue="";});window.onload=function(){o=JSON.parse('{"key":"start",'+
'"btn":[1,1,1,1],'+
'"sld":[70,30,40],'+
'"llm":[580, 270, 400],'+
'"clr":"#A000FF",'+
...
{if(buttonStates[num-1]==1){document.getElementById(elem.id+"_c").style.color="red";if(!first){buttonStates[num-1]=0};}else{document.getElementById(elem.id+"_c").style.color="green";if(!first){buttonStates[num-1]=0};}
...
function message(type,id,data){comm={"nam":type,"id":id,"data":data}

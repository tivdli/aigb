var ws;var gW="ws://"+window.location.hostname+"/ws";profile_name_climate=["Tropical","Dry","Temperate","Continental"];profile_name_stage=["Seedling","Vegetative","Budding","Ripening"];profile_name_type=["Common","Leaf","Fruit","Flower"];profile_name_mode=["Phase","Stable","Custom","Stress"];window.addEventListener("beforeunload",(event)=>{ws.close();event.preventDefault();event.returnValue="";});window.onload=function(){initWS();o=JSON.parse('{"key":"start",'+
'"btn":[1,1,1,1],'+
'"sld":[70,30,40],'+
'"llm":[580, 270, 400],'+
'"clr":"#A000FF",'+
'"fvl":[12,1.5],'+
'"ase":[55, 14, 70],'+
'"ain":[20, 110]}');p=JSON.parse('{"key":"profl","nam":136,"col":"#A000FF","onh":6,"ofh":22, "onm":0,"ofm":30,"fin":12,"fqy":1.5,"tpd":20,"tpn":12,"hmd":80,"hmn":90}');fillPage(o);updatePage();setProfileRead(p);fillProfiles();};function initWS(){ws=new WebSocket(gW);ws.onopen=onOpen;ws.onclose=onClose;ws.onmessage=onMessage;}
...
{if(buttonStates[num-1]==1){document.getElementById(elem.id+"_c").style.color="red";if(!first){buttonStates[num-1]=0};}else{document.getElementById(elem.id+"_c").style.color="green";if(!first){buttonStates[num-1]=1};}
if(!first){message("button",elem.id,buttonStates[num-1])};}else if(num==(buttonStates.length+1)){document.getElementById(elem.id+"_c").style.color="green";setTimeout(function(){document.getElementById(elem.id+"_c").style.color="black"},250);message("button",elem.id,1);}}
...
function message(type,id,data){comm={"n":type,"i":id,"d":data}
